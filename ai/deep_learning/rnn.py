...
class RNN:
    def __init__(self, model_type=None):
        ...
        if self.model_type is None:
            print(ValueError("Model type must be specified. Set model_type to 'lstm'"))
            self.model_type = "lstm"

        # Load data
        data, labels = load_data("../../data/processed/zemberek-5k.txt", group=False, group_size=2, unique=True)
        ...
        if self.model_type == 'lstm':
            model = self.lstm(train_vectors, embedding_matrix, embedding_dim)
        elif self.model_type == 'bi_lstm':
            model = self.bi_lstm(train_vectors, embedding_matrix, embedding_dim)
        elif self.model_type == 'gru':
            model = self.gru(train_vectors, embedding_matrix, embedding_dim)
        ...
        dot_img_file = '../../raporlama/images/lstm_model.png'
        plot_model(model, to_file=dot_img_file, show_shapes=True)
        """

    def bi_lstm(self, train_vectors, embedding_matrix, embedding_dim):
    ...
    def lstm(self, train_vectors, embedding_matrix, embedding_dim):
        model = keras.Sequential()
        model.add(Input(shape=(train_vectors.shape[1],)))
        model.add(Embedding(input_dim=embedding_matrix.shape[0], output_dim=embedding_dim,
                            embeddings_initializer=keras.initializers.Constant(embedding_matrix), trainable=True))
    ...
    def gru(self, train_vectors, embedding_matrix, embedding_dim):
        model = keras.Sequential()
        model.add(Input(shape=(train_vectors.shape[1],)))
        model.add(Embedding(input_dim=embedding_matrix.shape[0], output_dim=embedding_dim,
                            embeddings_initializer=keras.initializers.Constant(embedding_matrix), trainable=True))
